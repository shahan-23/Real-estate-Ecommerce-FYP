...
<?php 
    ...
    $numrows = $data->getNumRows("SELECT * FROM fav LEFT JOIN listing ON fav.pid = listing.id WHERE fav.user_id ='".$_SESSION["BuyerUserid"]."'"); 
    ...
    if(isset($_POST['sortby']) AND $_POST['sortby'] == '1'){

        $listing = $data->getData("SELECT *FROM fav LEFT JOIN listing ON fav.pid = listing.id WHERE fav.user_id ='".$_SESSION["BuyerUserid"]."' ORDER BY CAST(listing.price AS DECIMAL(18,2)) ASC LIMIT $offset, $rowsperpage");

    } elseif(isset($_POST['sortby']) AND $_POST['sortby'] == '2') {

        $listing = $data->getData("SELECT * FROM fav LEFT JOIN listing ON fav.pid = listing.id WHERE fav.user_id ='".$_SESSION["BuyerUserid"]."' ORDER BY CAST(listing.price AS DECIMAL(18,2)) DESC LIMIT $offset, $rowsperpage");

    } else {

        $listing = $data->getData("SELECT * FROM fav LEFT JOIN listing ON fav.pid = listing.id WHERE fav.user_id ='".$_SESSION["BuyerUserid"]."' ORDER BY listing.id DESC LIMIT $offset, $rowsperpage");
    ...
    <section class="pb-8 page-title shadow">
        <div class="container">
            <nav aria-label="breadcrumb">
                ...
                <h1 class="fs-30 lh-1 mb-0 text-heading font-weight-600">Favourites</h1>
            </nav>
        </div>
    </section>
            ...
            <div class="media p-4 border rounded-lg shadow-hover-1 pr-lg-8 mb-6 flex-column flex-lg-row no-gutters"
                ...
                <div class="col-lg-4 mr-lg-5 card border-0 hover-change-image bg-hover-overlay">
                    ...
                    <div class="card-img-overlay p-2 d-flex flex-column">
                        ...
                        <form action="" method="post">
                                              <input type="hidden" name="user" value="<?php echo $_SESSION["BuyerUserid"] ?>">
